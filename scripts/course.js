...
allbtn.addEventListener('click', () => {
    ...
    const allcredits = courses.reduce((sum, course) => sum + (course.credits || 0), 0);
    totalCredits.innerHTML = `Total credist for courses listed avove is ${allcredits} credits.`;
});
CSEbtn.addEventListener('click', () => {
    document.querySelector('.course-cards').innerHTML = '';
    const CSEcourses = courses.filter(course => course.subject === 'CSE');
    createCourseCard(CSEcourses);
     const allcredits = CSEcourses.reduce((sum, course) => sum + (course.credits || 0), 0);
    totalCredits.innerHTML = `Total credist for courses listed avove is ${allcredits} credits.`;
});
WDDbtn.addEventListener('click', () => {
    ...
    const allcredits = WDDcourses.reduce((sum, course) => sum + (course.credits || 0), 0);
    totalCredits.innerHTML = `Total credist for courses listed avove is ${allcredits} credits.`;
});
onpageLoading();
